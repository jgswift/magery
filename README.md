...
class Foo
{
    ...
    {
        $this->registerMagic('read', 'bar', function(){
...
```php
$this->registerMagic('write', 'bar', function(){
...
All events are registered globally but the registerMagic function is protected unless otherwise specified.

```php
<?php
class Foo
{
    use Magery\Mage {registerMagic as public;}   // allow public event registration
...
$foo->registerMagic('read', 'bar', function(){
    throw new \Exception('Don\'t touch my bar!');
});
...
$foo->registerMagic('call', 'bar', function(){
    sleep(1);
    return microtime();
},true);
...
$foo->registerMagic('get', 'bar', function(){
    return 'Baz';
});
...
$foo->registerMagic('set', 'bar', function($v){
    $this->baz = $v;
});
